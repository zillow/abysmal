...
def get_uncovered_lines(source_map, coverage_tuples):
    ...
    of coverage tuples produced by one or more calls to
    ...
    for idx_instruction, hit in enumerate(any(covered) for covered in zip(*coverage_tuples)):
    ...
    for line, coverage_type in line_to_coverage_type.items():
        ...
        lines = totally_uncovered_lines if coverage_type == 0 else partially_covered_lines
        if isinstance(line, tuple):
            lines.extend(list(range(line[0], line[1] + 1))) # multi-line statement
        else:
            lines.append(line) # single-line statement
    return CoverageReport(
        tuple(sorted(partially_covered_lines)),
        tuple(sorted(totally_uncovered_lines))
    )
